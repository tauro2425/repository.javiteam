...
class f4mProxyHelper():
    ...
    def playF4mLink(self,url,name,proxy=None,use_proxy_for_chunks=False, maxbitrate=0, simpleDownloader=False, auth=None, streamtype='HDS',setResolved=False,swf=None):
        try:
            ...
            progress.create('[COLOR lime][B]CARGANDO STREAMING Javi.Team[/B][/COLOR]')
            stream_delay = 1
            progress.update( 20, "", '[COLOR red][B]DESCODIFICANDO CANAL[/B][/COLOR]', "" )
            xbmc.sleep(stream_delay*2000)
            progress.update( 100, "", '[COLOR red][B]CARGANDO STREAMING Javi-Team[/B][/COLOR]', "" )
